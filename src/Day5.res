...
let calcRowCol = strArr => {
    strArr->Belt_Array.reduce((0, 127, 0, 7), 
    ((minRow, maxRow, minCol, maxCol), char)=>{
    switch char {
...
let seatIDSet = seatStrs
    ->Belt_Array.map(strToSeatID)
    ->Belt_SetInt.fromArray

let solutionP1 = seatIDSet
    ->Belt_SetInt.maximum
    ->Js.log

let isNearByMySeatID = (n) => {
    seatIDSet->Belt_SetInt.some(x => x - 2 == n) && 
    seatIDSet->Belt_SetInt.every(x => x - 1 != n)}

let solutionP2 = seatIDSet
