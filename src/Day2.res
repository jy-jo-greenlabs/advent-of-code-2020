type pwInfoSetP1 = {min:int, max:int, ch:string, word:string}
...
let solution = 
parsedList->Belt_Array.map(convertToObj)->Belt.Array.keep(isValid)->Belt.Array.length
...
let solutionP2 = parsedList->
Belt_Array.map(convertToP2)->Belt.Array.keep(isValidP2)->Belt.Array.length
